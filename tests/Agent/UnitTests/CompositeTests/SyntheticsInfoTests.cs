...
namespace CompositeTests
{
    ...
    [TestFixture(true)]
    [TestFixture(false)]
    internal class SyntheticsInfoTests
    {
        private static CompositeTestAgent _compositeTestAgent;

        private const string SyntheticsHeaderKey = "X-NewRelic-Synthetics";
        private const string SyntheticsInfoHeaderKey = "X-NewRelic-Synthetics-Info";

        private IAgent _agent;

        private bool _isDistributedTracingEnabled;

        public SyntheticsInfoTests(bool isDistributedTracingEnabled)
        {
            _isDistributedTracingEnabled = isDistributedTracingEnabled;
        }

        private string GetInfoHeader(string type, string initiator, IDictionary<string, string> attributes = null)
        {
            if (attributes == null)
            {
                attributes = new Dictionary<string, string>
                {
                    { "exampleAttribute", "value1" },
                    { "exampleAttribute2", "value2" },
                };
            }
            var header = new SyntheticsInfoHeader(1, type, initiator, attributes);
            return header.TryGet();
        }
        ...
        [Test]
        public void synthetics_info_attributes_when_header_is_present()
        {
            // ARRANGE
            var encodingKey = "foo";
            var version = 1;
            ...
            var requestHeaders = new List<KeyValuePair<string, string>>()
            {
                ...
                new KeyValuePair<string, string>(SyntheticsInfoHeaderKey, GetInfoHeader(type, initiator))
            };
            ...
            var expectedEventAttributes = new List<ExpectedAttribute>
            {
                ...
                new ExpectedAttribute {Key = "nr.syntheticsType", Value = type},
                new ExpectedAttribute {Key = "nr.syntheticsInitiator", Value = initiator},
                new ExpectedAttribute {Key = "nr.syntheticsexampleAttribute", Value = "value1"},
                new ExpectedAttribute {Key = "nr.syntheticsexampleAttribute2", Value = "value2"},
            };
            var expectedTraceAttributes = new List<ExpectedAttribute>
            {
                ...
                new ExpectedAttribute {Key = "synthetics_type", Value = type},
                new ExpectedAttribute {Key = "synthetics_initiator", Value = initiator},
                new ExpectedAttribute {Key = "synthetics_exampleAttribute", Value = "value1"},
                new ExpectedAttribute {Key = "synthetics_exampleAttribute2", Value = "value2"},
            };
        ...
        [Test]
        public void synthetics_info_attributes_when_no_header_is_present()
        {
            // ARRANGE
            var encodingKey = "foo";
            ...
            var unexpectedEventAttributes = new List<string>
            {
                ...
                "nr.syntheticsexampleAttribute2"
            };
            var unexpectedTraceAttributes = new List<string>
            {
                ...
                "synthetics_exampleAttribute",
                "synthetics_exampleAttribute2"
            };
            var transactionEvent = _compositeTestAgent.TransactionEvents.First();
            var transactionTrace = _compositeTestAgent.TransactionTraces.First();
            NrAssert.Multiple(
        ...
        [Test]
        public void synthetics_info_attributes_when_no_headers_are_provided()
        {
            // ARRANGE
            var encodingKey = "foo";
            _compositeTestAgent.ServerConfiguration.TrustedIds = new long[] { 123 };
        ...
        [Test]
        public void synthetics_info_header_is_generated_when_outbound_request_is_made()
        {
            // ARRANGE
            var encodingKey = "foo";
            var version = 1;
            ...
            var infoHeaders = headers[SyntheticsInfoHeaderKey];
        ...
        public void synthetics_header_can_be_decoded_from_outbound_request()
        {
            ...
            SyntheticsInfoHeader infoHeader = syntheticsHeaderHandler.GetInfoHeaders(headers, GetHeaderValue);
            ...
            NrAssert.Multiple(
                ...
                () => Assert.AreEqual(infoHeader.Type, type),
                () => Assert.AreEqual(infoHeader.Initiator, initiator)
            );
