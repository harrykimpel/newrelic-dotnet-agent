...
function Copy-ExtensionsOther {
    ...
    if ($Type -like "Core"){
        Copy-Item -Path "$RootDirectory\src\_build\AnyCPU-$Configuration\NewRelic.Core\net8.0-ILRepacked\NewRelic.Core.dll" -Destination "$Destination\extensions" -Force 
        Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Agent\Parsing\bin\$Configuration\net8.0\NewRelic.Parsing.dll" -Destination "$Destination\extensions" -Force 
    }
}
...
function Copy-AgentRoot {
    ...
    if ($Type -like "Core") {
        Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Agent\Extensions\NewRelic.Agent.Extensions\bin\$Configuration\net8.0\NewRelic.Agent.Extensions.dll" -Destination "$Destination" -Force 
        Copy-Item -Path "$RootDirectory\src\_build\AnyCPU-$Configuration\NewRelic.Agent.Core\net8.0-ILRepacked\NewRelic.Agent.Core.dll" -Destination "$Destination" -Force 
    ...
    if ($Linux) {
        if ($Architecture -like "x64") {
            Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Home\bin\$Configuration\net8.0\profiler\linux_x64\libNewRelicProfiler.so" -Destination "$Destination" -Force 
        }
        if ($Architecture -like "ARM64") {
            Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Home\bin\$Configuration\net8.0\profiler\linux_arm64\libNewRelicProfiler.so" -Destination "$Destination" -Force 
        }
    }
    else {
        if ($Type -like "Framework") {
        ...
        if ($Architecture -like "x64" ) {
            Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Home\bin\$Configuration\net8.0\profiler\x64\NewRelic.Profiler.dll" -Destination "$Destination" -Force 
        }
        else {
            Copy-Item -Path "$RootDirectory\src\Agent\NewRelic\Home\bin\$Configuration\net8.0\profiler\x86\NewRelic.Profiler.dll" -Destination "$Destination" -Force 
        }
    }
...
function Copy-AgentApi {
    ...
    if ($Type -like "Core") {
        Copy-Item -Path "$RootDirectory\src\_build\AnyCPU-$Configuration\NewRelic.Api.Agent\net8.0\NewRelic.Api.Agent.dll" -Destination "$Destination" -Force 
    }
}
